...
def main():
    if platform.system() == 'Linux':
        ...
        out_path = f'C:/Users/Dylan/OneDrive - UCLA IT Services/Research/Saclay/sPHENIX/Vernier_Scan/Analysis_Note/Cross_Checks/'
    ...
    for scan_date in vernier_scan_dates:
        ...
        for orientation in orientations:
            ...
            # bw_df_amps = bw_fit_df['amplitude'].to_numpy()
            ...
            width_str = f'MBD Measured Fit Width: {pmeas_cw[1]} μm\nNumerical Fit Width: {pmeas_sim[1]} μm'

            # Plot
            fig, ax = plt.subplots()
            ax.plot(offsets, rates, marker='o', c='k', ls='none', label='MBD Measured Rates')
            ax.plot(xs_plot, vernier_scan_fit_bkg(xs_plot, *popt_cw), '--', c='k', label='MBD Fit')
            ax.plot(bw_df_offsets, bw_df_lumis, marker='o', c='r', ls='none', label=r'Numerical Integration $\mathcal{L}_{\rm{naked}}$')
            ax.plot(xs_plot, vernier_scan_fit_bkg(xs_plot, *popt_sim), '--', c='r', label='Numerical Integration Fit')
            ax.set_xlabel('Set Position [μm]')
            ax.set_ylabel('Rate [Hz]')
            ax.annotate(width_str, (0.02, 0.97), xycoords='axes fraction', ha='left', va='top', fontsize=10,
                        bbox=dict(facecolor='white', alpha=0.5, boxstyle='round'))
            ax.legend()
            ax.set_ylim(bottom=0, top=np.max(rates) * 1.2)
            ax.set_title(f'{scan_date} {orientation} Set Position vs Rate')
            fig.tight_layout()
            fig.subplots_adjust(left=0.085, right=0.995, bottom=0.095, top=0.95)

            if out_path:
                fig.savefig(f'{out_path}{scan_date}_{orientation}_set_pos_vs_rate.png')
                fig.savefig(f'{out_path}{scan_date}_{orientation}_set_pos_vs_rate.pdf')
